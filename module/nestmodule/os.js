...
console.log(`os.uptime(): ${os.uptime()}`) // 운영체제 부팅 이후 흐른 시간(초)을 보여줌, process.uptime()의 경우 노드의 실행시간
console.log(`os.hostname(): ${os.hostname()}`) // 컴퓨터의 이름을 보여줌
console.log(`os.release(): ${os.release()}`) // 운영체제 버전을 보여줌
console.log('-------------------경로 정보-------------------')
console.log(`os.homedir(): ${os.homedir()}`) // 홈 데릭토리 경로를 보여줌
console.log(`os.tmpdir(): ${os.tmpdir()}`) // 임시 파일 저장 경로를 보여줌
console.log('-------------------CPU 정보-------------------')
console.log(`os.cpus(): ${os.cpus()}`) // 컴퓨터 코어 정보를 보여줌
console.log(`os.cpus().length: ${os.cpus().length}`) // 코어의 갯수
console.log('-------------------메모리 정보-------------------')
